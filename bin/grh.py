...
def runit(args):
    ...
    print("Thank you for using grh 🎉. Please call again 👋")
    sys.exit(0)
...
def main():
    ...
    histfile = os.path.join(os.path.expanduser("~"), ".grh_history")
    try:
...
def _get_available_commits(include, excludes):
    ...
    # print(len(commits), "that are not nearly identical")
...
#     if len(v) > 1:
#         diffs = [clean_git_show(c) for c in v]
#         same = True
#         for d in diffs[1:]:
#             if d != diffs[0]:
#                 # run_meld(d, diffs[0])
...
#         else:
#             c2 += 1
#         print(f"{c2} are identical, while {c1} differ", end="\r")
#         sys.stdout.flush()
#     # if c1 + c2 > 10:
#     #     break
